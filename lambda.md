...
Lambda 함수는 AWS의 대표적인 서버리스 서비스입니다. IoT Greengrass는 AWS Lambda를 Edge device에서 실행할 수 있는 환경을 제공함으로, S3와 같은 저장소나 DynamoDB/Timestream 같은 데이터베이스를 Greengrass에서 Lambda를 통해 편리하게 활용할 수 있습니다. 
...
Lambda의 event source로 local publish/subscribe 메시지들과 IoT Core MQTT 메시지들를 이용할 수 있습니다. 이를 위해 Topic Type을 설정하여야 하는데, Type에는 "Local publish/subscribe"와 "AWS IoT Core MQTT"가 있습니다. 
